// JSLint options:
/*global document, window, navigator, setInterval, clearInterval, clearTimeout, setTimeout, location, jQuery, $, console */
var defWidth;
var yHieghts = [];
var hDex =[0,0];
var i =0;
...
function error(code, stop) {
	var msg = 'SOCR_JSCharts error #' + code + ': www.SOCR.ucla.edu' + code;
	if (stop) {
		throw msg;
	} else if (win.console) {
		console.log(msg);
...
// check for a custom SOCR_JSChartsAdapter defined prior to this file
var globalAdapter = win.SOCR_JSChartsAdapter,
	adapter = globalAdapter || {},

	// Utility functions. If the SOCR_JSChartsAdapter is not defined, adapter is an empty object
 ...
 * SOCR_JSCharts reverts to the built-in jQuery adapter.
 */
...
if (!globalAdapter && win.jQuery) {
	...
	fireEvent = function (el, type, eventArguments, defaultFunction) {
		...
		// Remove warnings in Chrome when accessing layerX and layerY. Although SOCR_JSCharts
	...
	// extend some methods to check for elem.attr, which means it is a SOCR_JSCharts SVG object
		...
		if (elem.tagName === 'SPAN') {
			...
			if (currentTextTransform !== wrapper.cTT) { // do the calculations and DOM access only if properties changed
				...
				if (defined(rotation)) {
					...
					// Test case: http://SOCR.ucla.edu/tests/?file=text-rotation
					css(elem, {
...
SVGRenderer.prototype = {
	...
	init: function (container, width, height, forExport) {
		...
		renderer.setSize(width, height, false); //svg tag size change
	...
	/**
	 * Create a group
	 * @param {String} name The group will be given a class name of 'SOCR_JSCharts-{name}'.
	...
	fontMetrics: function (fontSize) {
		...
		// Applies to the default font family. http://jsfiddle.net/SOCR_JSCharts/7xvn7/
...
var VMLElement = {
	...
	clip: function (clipRect) {
		...
		if (parentNode && parentNode.className === 'SOCR_JSCharts-tracker' && !docMode8) {
			css(element, { visibility: HIDDEN });
		}
	...
	color: function (color, elem, prop) {
		...
		if (fillType) {
			...
			// Radial (circular) gradient
				...
				// pie:       http://jsfiddle.net/SOCR_JSCharts/66g8H/
				// reference: http://jsfiddle.net/SOCR_JSCharts/etznJ/
				// http://jsfiddle.net/SOCR_JSCharts/XRbCc/
				// http://jsfiddle.net/SOCR_JSCharts/F3fwR/
				// TODO:
				...
				//}
				fillAttr = 'src="http://code.SOCR.ucla.edu/gfx/radial-gradient.png" ' +
					'size="' + size + ',' + size + '" ' +
					'origin="0.5,0.5" ' +
					'position="' + cx + ',' + cy + '" ' +
					'color2="' + color2 + '" ';
...
Axis.prototype = {
	...
	getPlotLinePath: function (value, lineWidth, old) {
		var axis = this,
			...
			skip,
			transB = axis.transB;
		console.log('divCoutn',divCount,'axisLeft', axisLeft,'axisTop',axisTop,'cHight',cHeight,'cWidth',cWidth, 'transB',transB);
		...
		if (isNaN(translatedValue)) { // no min or max
			...
			if(divCount == 1){
				...
				y1=y2=yHieghts[hDex[1]];
				if(hDex[0]<=hDex[1]){hDex[1]=0;}
				else{hDex[1]++;}
				x1 = 0;
			}
...
Chart.prototype = {
	...
	getChartSize: function () {
		...
		if((divCount == 1 && !defWidth)){
		chart.chartWidth = optionsChart.width/2 || chart.containerWidth/2 || 300;
		defWidth = chart.chartWidth;
		}
		else{
		if(defWidth <= ($(window).width()-$('td#dataTd').width())/divCount){
			chart.chartWidth = defWidth;
		}
		else{
			chart.chartWidth = optionsChart.width/2 || chart.containerWidth/2 || 300;
			defWidth = chart.chartWidth;
	...
	getContainer: function () {
		...
		chart.container = container = createElement('span', {
		...
		chart.renderer =
			optionsChart.forExport ? // force SVG, used for SVG export
				new SVGRenderer(container, chartWidth, chartHeight, true) : //change
				new Renderer(container, chartWidth, chartHeight); //change
	...
	initReflow: function () {
		...
		var reflowTimeout;
		function reflow(e) {
			if($('#violin').width() > ($(window).width()-$('td#dataTd').width())){
			var width =($(window).width()-$('td#dataTd').width())/divCount,//optionsChart.width || adapterRun(renderTo, 'width'),
				height = optionsChart.height || adapterRun(renderTo, 'height'),
				target = e ? e.target : win; // #805 - MooTools doesn't supply e
			}
			else{
				var width =optionsChart.width/2 || adapterRun(renderTo, 'width'),
				height = optionsChart.height || adapterRun(renderTo, 'height'),
				target = e ? e.target : win; 
			}
				//width = width/divCount;
				defWidth = width;
				$('#violin').width(width*divCount+ divCount);
				$('#wrapper').width(width*divCount+ divCount);
			...
			if (width && height && (target === win || target === doc)) {
				...
				chart.containerWidth = width*divCount + divCount;
				chart.containerHeight = height;
			}
			LabsArr = $('g.SOCR_JSCharts-axis');
if(divCount!=1 && LabsArr.length !=0){
	var yLabs = $(LabsArr[2*(divCount-1)]).children('text');
	for(var i =0; i<yLabs.length;i++){
	$(yLabs[i]).children()[0].textContent ='';
	}
}
		}
	...
	firstRender: function () {
		...
		chart.getContainer();//i think determines which div has the stuff created in it. hello

		// Run an early event after the container and renderer are established
		fireEvent(chart, 'init');

		// Initialize range selector for stock charts
		if (SOCR_JSCharts.RangeSelector && options.rangeSelector.enabled) {
			chart.rangeSelector = new SOCR_JSCharts.RangeSelector(chart);
		}
		...
		if (SOCR_JSCharts.Scroller && (options.navigator.enabled || options.scrollbar.enabled)) {
			chart.scroller = new SOCR_JSCharts.Scroller(chart);
		}
		...
		chart.cloneRenderTo(true); //Hello setTimeout here
...
Series.prototype = {
	 ...
	 * symbols through SOCR_JSCharts.seriesTypes[type].prototype.drawLegendSymbols.
	 * 
	...
	setData: function (data, redraw) {
		...
		if (dataLength > turboThreshold) {
			...
			if (isNumber(firstPoint)) { // assume all points are numbers
				...
				error(12); // SOCR_JSCharts expects configs to be numbers or arrays in turbo mode
			}*/
		} else {
			for (i = 0; i < dataLength; i++) {
				pt = { series: series };
	 ...
	 * unused points and optionally grouped in SOCR_JSCharts Stock.
	 */
