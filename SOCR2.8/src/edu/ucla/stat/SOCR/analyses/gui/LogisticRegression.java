	...
	public void doAnalysis(){
		...
		if (dependentIndex < 0 || independentIndex < 0 || independentLength == 0) {
			...
			if (cellValue != null && !cellValue.equals("")) {
                ...
                double originalData[][] = new double[originalRow][originalColumn];
			...
			originalData[k][j] = Double.parseDouble((String)dataTable.getValueAt(k, j));
                    }
		}  
                ...
                System.out.println("Dependent Index is: " + dependentIndex + "\n");
		...
		for (int k = 0; k < originalRow; k++){
                    int x = 0;
                        for (int j = 0; j < originalColumn-1; j++){
                            if (j != dependentIndex){
                                covariate[k][j] = originalData[k][x];
                                x++;
                            }
                            else{
                                x++;
		...
		independentHeaderArray = new String[independentVar.length];
			...
			varIndexList[i] = varIndex;
                ...
                Arrays.sort(varIndexList); // sort from smallest to largest to keep the same order, regardless of user selection
                
		for (int i = 0; i < independentVar.length; i++) {
			varIndex = ( (Integer)independentList.get(i)).intValue();
			independentHeader = columnModel.getColumn(varIndexList[i]).getHeaderValue().toString().trim();
		...
		for (int i = 0; i < betas.length; i++) {
			...
			if (i==0) resultPanelTextArea.append(" "+ result.getFormattedDouble(betas[i]));
                            ...
                            if (independentHeaderArray.length-i == -1)
                                break;
                            resultPanelTextArea.append(" +"+ result.getFormattedDouble(betas[i])+"*"+independentHeaderArray[i-1]);
                        }
		}
		resultPanelTextArea.append(".\n\n");
