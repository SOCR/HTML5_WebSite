...
public class FisherExact extends Analysis implements MouseListener, ActionListener, KeyListener, MouseMotionListener, PropertyChangeListener {
	...
	private JComboBox<String> rowNumberCombo = new JComboBox(new String[]{"2","3","4","5", "6", "7", "8", "9", "10"}); //
	private JComboBox<String> colNumberCombo = new JComboBox(new String[]{"2","3","4","5", "6", "7", "8", "9", "10"}); //
	...
	private static final int MAX_ROW_SIZE = 10;	// IVO UPDATED FROM 5 --> 10
	...
	public void init(){
		...
		showVisualize= false;
		super.init();
		analysisType = AnalysisType.FISHER_EXACT;
		analysisName = "Fisher's Exact Test";
...
*/
		...
		//this.rowNumber = 2; this.colNumber = 2;
		/*
		if (!hasInput) {
			try {
	...
	public void updateResults(){
		...
		int[] colSum = null;
		int grandTotal = 0;
		double chiStat = 0, pValueCutoff = 0, pValue1TailCutoff=0, pValue2TailCutoff=0;
		try {
		...
		try {
			pValue1TailCutoff = result.get1TailPCutoff();
		} catch (Exception e) {
			////////System.out.println("Exception = " + e);
		}
		try {
			pValue2TailCutoff = result.get2TailPCutoff();
		} catch (Exception e) {
			////////System.out.println("Exception = " + e);
		}
		...
		*/
		try {
			...
			//System.out.println("rowSum Exception = " + e);
		}
		try {
			colSum = result.getColSum();
		} catch (Exception e) {
			//System.out.println("colSum Exception = " + e);
		}
		...
		resultPanelTextArea.append("\tResults of Fisher's Exact Test\n" );
		...
		if (pValueCutoff == FisherExactResult.INVALID_P_VALUE_CUTOFF) {
			...
			resultPanelTextArea.append("\n\n\tP-Value Cutoff = " + result.getFormattedDouble(pValueCutoff));
		}
		
		if (pValue2TailCutoff == FisherExactResult.INVALID_P_VALUE_CUTOFF_2_TAIL) {
			resultPanelTextArea.append("\n\n\t2-Tail P-Value is not avaialble "
					+"due to the entries being too large.");
		}
		else {
			resultPanelTextArea.append("\n\n\t1-Tail P-Value = " + 
					result.getFormattedDouble(pValue1TailCutoff));
			resultPanelTextArea.append("\n\n\t2-Tail P-Value = " + 
					result.getFormattedDouble(pValue2TailCutoff));
